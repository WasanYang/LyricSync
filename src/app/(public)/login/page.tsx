...
export default function LoginPage() {
  const { signInWithGoogle, signInWithFacebook, signInAnonymously } =
    useAuth();
  const router = useRouter();
  const { toast } = useToast();

  const handleSignIn = async (method: 'google' | 'facebook' | 'guest') => {
    try {
      if (method === 'google') {
        ...
        <div className='w-full max-w-sm flex flex-col items-center text-center'>
          ...
          <div className='w-full space-y-4'>
            ...
            <Button
              ...
              size='lg'
              className='w-full bg-white text-[#1877F2] hover:bg-gray-100'
            >
