...
export default function NotificationForm({
    ...
    if (isEditing && notificationId) {
      getNotification(notificationId).then((notif) => {
        if (notif) {
          ...
          const scheduledAtDate = notif.scheduledAt
          ...
          reset({
            ...
            scheduledHour: notif.scheduledAt
              ? format(scheduledAtDate, 'HH')
              : '09',
            scheduledMinute: notif.scheduledAt
              ? format(scheduledAtDate, 'mm')
              : '00',
          });
        }
        setIsLoading(false);
  ...
  async function onSubmit(data: NotificationFormValues) {
    ...
    const finalData: Omit<AppNotification, 'createdAt'> = {
      ...
      recipientIds:
        data.recipientType === 'SPECIFIC_USERS'
          ? data.recipientIds
              ?.split(',')
              .map((s) => s.trim())
              .filter(Boolean)
            ...
            <p className='text-muted-foreground'>
              ...
              {isSent && (
                <span className='text-destructive font-bold ml-2'>
                  (This notification has been sent and cannot be edited)
                </span>
              )}
            </p>
          </div>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormDescription>
                          Separate multiple user IDs with a comma (,)
                        </FormDescription>
                        <FormMessage />
                      </FormItem>
                    )}
