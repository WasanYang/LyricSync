...
const LyricLineDisplay = ({
  ...
  const parsedLine = useMemo(
    () => parseLyricsForDisplay(line.text),
    [line.text]
  );
  ...
  if (isSectionHeader) {
    return (
      <p className='italic text-muted-foreground my-4'>{`[ ${line.text.substring(
        1,
        line.text.length - 1
      )} ]`}</p>
    );
  ...
  if (!showChords || !hasChords) {
    return (
      <div className='min-h-[1.5rem] whitespace-pre-wrap'>{cleanLyricText}</div>
    );
  }
